...
const CreateItemModal = ({ open, handleClose, id}) => {
    ...
    const addNewItem = async () => {
        if(!data.sku) toast.error("SKU is required");
            ...
            const response = await postData('/api/item', {...data, product: id});
            if(response.success) window.location.reload();
            else toast.error(response.response.data.errors[0])
        }
    }
    ...
    return <BaseModal open={open} handleClose={handleClose}>
        <div className="flex flex-col gap-5 w-[400px]">
            ...
            />
            <CustomButton onClick={addNewItem}>ADD ITEM</CustomButton>
        </div>
